...
| Prioridade | ID | Finding | Impacto | Próxima ação |
|---|---|---|---|---|
| CRÍTICA | `RB-FW-001` | DROP GERAL da chain input está disabled | Sem um drop final ativo, tráfego não correspondido na chain input é aceito por padrão. Como o export não mostra restrição explícita de SSH/Winbox/WWW, o management plane da RB precisa ser auditado antes de ser considerado endurecido. | Executar `/ip service print detail`; restringir serviços à VLAN90/Tailscale; validar regras necessárias e então ativar política default-deny no input com rollback/Safe Mode. |
| ALTA | `RB-FW-002` | Não há política default-deny explícita na chain forward | A VLANização existe, mas a configuração exportada não demonstra isolamento L3 entre VLANs; regras como MGMT→ALL ficam redundantes se o restante do forward é aceito por padrão. | Definir matriz de fluxos inter-VLAN (30<->60, MGMT->ALL, guest->Internet/DNS etc.) e implementar default-deny gradual. |
| ALTA | `RB-DHCP-001` | Pools DHCP ativos sobrepõem endereços estáticos de infraestrutura | Pools 30/60/90 começam em .2 e vão até .254, sobrepondo hosts PVE, Tailscale, Zabbix, Pi-hole e o switch (10.100.90.99). | Redesenhar pools com faixas reservadas, por exemplo estáticos baixos e DHCP em faixa separada; revisar leases e reservas antes de alterar. |
| MÉDIA | `RB-SNMP-001` | Origem autorizada da community SNMP difere da regra de firewall | A community exportada limita `addresses=10.100.0.6/32`, enquanto a regra input permite UDP/161 de `10.100.30.60` (Zabbix). | Confirmar IP real do poller; alinhar origem e migrar a RB para SNMPv3 authPriv. |
| MÉDIA | `RB-NTP-001` | Cliente NTP da RB está desabilitado | O relógio pode derivar e comprometer correlação de logs/eventos. | Habilitar cliente NTP/SNTP com duas fontes confiáveis e validar sincronismo. |
| MÉDIA | `RB-DNS-001` | Política DNS é inconsistente entre VLANs | VLANs 30/60/90 recebem 1.1.1.1/8.8.8.8; somente VLAN130 recebe Pi-hole 10.100.60.71/72 e tem redirecionamento anti-bypass. | Decidir política única; se o objetivo é observabilidade/bloqueio central, entregar Pi-hole para todas as VLANs aplicáveis e documentar exceções. |
| MÉDIA | `RB-DISC-001` | Neighbor discovery usa `discover-interface-list=!interfaces-secure` | A expressão está inversa ao nome da lista e merece validação para evitar descoberta em interfaces não confiáveis. | Validar com `/ip neighbor discovery-settings print` e limitar descoberta somente às interfaces administrativas necessárias. |
| BAIXA | `RB-POOL-001` | Há pools DHCP não utilizados/legados | `dhcp_pool1`, `pool-wifi`, `dhcp_pool4` e `dhcp_pool6` não são referenciados pelos DHCP servers ativos no export. | Confirmar ausência de dependências e remover resíduos em mudança separada. |
| BAIXA | `RB-KNOCK-001` | Port knocking adiciona `rede-suporte`, mas não há regra de accept correspondente | As portas 3399/6699 populam listas, porém o export não mostra consumo de `rede-suporte` para liberar serviço. | Revisar intenção; concluir o fluxo ou remover regras órfãs. |
| BAIXA | `SW-CLEAN-001` | ACL numérica 10 permaneceu após remoção do SNMPv2c | `access-list 10 permit 10.100.30.60` ainda existe, mas SNMP usa `SNMP_ZABBIX_ONLY`. | Remover ACL 10 após confirmar nenhuma referência. |
| BAIXA | `SW-BANNER-001` | Banner MOTD está truncado/malformado | O running-config contém `banner motd` interrompido (`oggin ^` / texto truncado), enquanto o banner login está íntegro. | Remover o MOTD ou recriá-lo com delimitador limpo; manter uma única mensagem legal consistente. |
| REVISÃO | `SW-AAA-001` | AAA new-model não está habilitado | Autenticação é local e protegida por ACL, o que é funcional; AAA centralizado não é requisito atual. | Manter como decisão arquitetural; considerar AAA/TACACS+/RADIUS somente se houver servidor e necessidade operacional. |

## 8. Sequência recomendada para próxima padronização

1. **RB management plane:** coletar `/ip service print detail`, restringir SSH/Winbox/WWW e validar origem Tailscale/MGMT.
2. **RB firewall input:** criar política default-deny segura em Safe Mode, preservando DHCP/DNS/ICMP/NTP/SNMP/PPPoE conforme necessário.
3. **RB firewall forward:** definir matriz de comunicação entre VLANs e só então aplicar default-deny.
4. **DHCP:** separar faixa dinâmica de IPs estáticos e remover pools legados após validação.
5. **RB SNMP:** corrigir origem do poller e migrar para SNMPv3 authPriv.
6. **RB NTP:** habilitar cliente NTP/SNTP e validar timezone/logs.
7. **DNS:** decidir se Pi-hole será padrão para 30/60/90/130 e alinhar DHCP/NAT anti-bypass.
8. **Cisco cleanup:** remover ACL 10 órfã e corrigir/remover banner MOTD truncado.
9. **Reinventário:** coletar estado operacional (status/speed, LLDP, STP, ARP/MAC, SNMP, firewall counters) e atualizar o repositório.

## 9. Regras operacionais / guardrails

- Cisco remoto: `write memory` do estado conhecido → `reload in 10` → alteração → validar nova sessão → `reload cancel` → `write memory`.
- MikroTik remoto: export + backup → Safe Mode → alteração → validar nova sessão → sair do Safe Mode normalmente.
- Não criar segundo enlace L2 Cisco↔MikroTik no desenho atual; para redundância física, redesenhar STP (preferencialmente MSTP nos dois lados).
- Não publicar configs brutas sem sanitização de credenciais, hashes, communities e identificadores de acesso WAN.

## 10. Fontes / rastreabilidade

- `Texto colado(2).txt` — SHA-256 `0902997f0dbfea83295e86c4af2e2179614244b3ab300bed0ebeb5cc761c4670`
- `base_line-RT.rsc` — SHA-256 `a3430a3d7a7b2b99e58225b079e1ab83ec7fea0b54ad99e0dc3488deae4e54f8`
- Estado operacional e validações complementares foram confirmados durante a sessão de padronização (SNMPv3 verde no Zabbix, NTP sincronizado, Gi1/0/15 em 1 Gb/s e acesso SSH validado).

## 11. Dados que devem ser coletados no próximo inventário

```text
Cisco: show version; show interfaces status; show interfaces trunk; show vlan brief; show spanning-tree; show lldp neighbors detail; show ip ssh; show snmp user; show logging; show ntp status; show access-lists
MikroTik: /interface print detail; /interface bridge print detail; /interface bridge port print detail; /interface bridge vlan print detail; /ip service print detail; /ip firewall filter print detail stats; /ip firewall nat print detail stats; /ip pool print detail; /ip dhcp-server lease print detail; /snmp print detail; /snmp community print detail; /system ntp client print detail
```
